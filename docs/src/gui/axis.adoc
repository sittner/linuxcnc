...
* Tool Touch Off(((AXIS GUI, Tool Touch Off)))
...
(((AXIS GUI, Soft Limits)))(((Soft Limits)))
[[fig:soft-limits]]
.Soft Limits
...
(((AXIS GUI, Manual Tool Change)))(((Manual Tool Change)))
LinuxCNC includes a realtime HAL component called 'manualtoolchange',
which together with the 'manualtoolchange_ui' program shows a window
prompt telling you what tool is expected when a 'M6' command is issued.
After the OK button is pressed, execution of the program will continue.

The manualtoolchange component includes a HAL pin for a button that
can be connected to a physical button to complete the tool change and
remove the window prompt (manualtoolchange.change_button).

The HAL configuration file 'lib/hallib/axis_manualtoolchange.hal'
shows the HAL commands necessary to use this component.

manualtoolchange can be used even when AXIS is not used as the GUI.
...
(((AXIS GUI, Virtual Control Panel)))
