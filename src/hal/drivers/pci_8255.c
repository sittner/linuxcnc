...
static void pci_8255_read(struct port *inst, long period);
static void pci_8255_write(struct port *inst, long period);
...
static int export(char *prefix, struct port *inst, int ioaddr, int dir) {
    ...
    r = hal_export_funct(buf, (void(*)(void *inst, long))pci_8255_read, inst, 0, 0, comp_id);
    if(r != 0) return r;
    rtapi_snprintf(buf, sizeof(buf), "%s.write", prefix);
    r = hal_export_funct(buf, (void(*)(void *inst, long))pci_8255_write, inst, 0, 0, comp_id);
    if(r != 0) return r;
...
static void write_all(struct state *inst, long period) {
	...
	pci_8255_write(&inst[i].ports[0], period);
	pci_8255_write(&inst[i].ports[1], period);
	pci_8255_write(&inst[i].ports[2], period);
    }
}
...
static void read_all(struct state *inst, long period) {
    int i;
    for(i=0; i<count; i++) {
	pci_8255_read(&inst[i].ports[0], period);
	pci_8255_read(&inst[i].ports[1], period);
	pci_8255_read(&inst[i].ports[2], period);
    }
}
