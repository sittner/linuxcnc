...
rtapi_print_msg(RTAPI_MSG_INFO, "Libgpiod is %i\n", LIBGPIOD_VER);
    ...
    inst->gpio = rtapi_malloc(sizeof(hal_gpio_t));
    if (inst->gpio == 0) {
...
# else
	...
	for (i = 0; i < gpio->in_chips[c].num_lines; i++){
	    ...
	    retval += hal_pin_bit_newf(HAL_OUT, &(gpio->in_chips[c].hal[i].value), inst->comp_id, "hal_gpio.%s-in", line_name);
	    retval += hal_pin_bit_newf(HAL_OUT, &(gpio->in_chips[c].hal[i].value_not), inst->comp_id, "hal_gpio.%s-in-not", line_name);
	}
	if (retval < 0){
...
# else
	...
	for (i = 0; i < gpio->out_chips[c].num_lines; i++){
	    ...
	    retval += hal_pin_bit_newf(HAL_IN, &(gpio->out_chips[c].hal[i].value), inst->comp_id, "hal_gpio.%s-out", line_name);
	}
	if (retval < 0){
    ...
    retval += hal_export_funct(hal_name, hal_gpio_read, inst, 0, 0, inst->comp_id);
    rtapi_snprintf(hal_name, HAL_NAME_LEN, "hal_gpio.write");
    retval += hal_export_funct(hal_name, hal_gpio_write, inst, 0, 0, inst->comp_id);

    if (inst->reset_active){
	gpio->reset_ns = hal_malloc(sizeof(hal_u32_t));
	rtapi_snprintf(hal_name, HAL_NAME_LEN, "hal_gpio.reset");
	retval += hal_param_u32_newf(HAL_RW, gpio->reset_ns, inst->comp_id, "hal_gpio.reset_ns");
	retval += hal_export_funct(hal_name, hal_gpio_reset, inst, 0, 0, inst->comp_id);
    }
    if (retval < 0){
...
static void hal_gpio_write(void *arg, long period)
{
    ...
    inst->last_reset = rtapi_get_clocks();
}
