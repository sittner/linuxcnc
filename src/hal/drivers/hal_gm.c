...
//Methods exported to HAL
  static void gm_read(void *arg, long period);
  static void gm_write(void *arg, long period);
...
static int ExportFunctions(void *arg, int comp_id, int boardId)
{
	...
	error = hal_export_funct(str, gm_write, device, 1, 0, comp_id);

	if(error == 0)
	{
		rtapi_snprintf(str, sizeof(str), "gm.%d.read", boardId);
		error = hal_export_funct(str, gm_read, device, 1, 0, comp_id);
	}
