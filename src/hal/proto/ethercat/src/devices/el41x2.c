/**
 * @file el41x2.c
 * @brief Driver implementation for Beckhoff EL41x2 2-channel analog output terminals.
 *
 * Handles initialization and real-time write processing for the EL4102,
 * EL4112, EL4122, and EL4132 dual-channel extended-range analog output
 * terminals.  Unlike the EL40x2, channel PDO entries are mapped via CoE
 * objects 0x3001 and 0x3002 rather than the 0x70xx scheme.
 *
...
/**
 * @brief Per-channel HAL data for a single EL41x2 output channel.
 *
...
/**
 * @brief Top-level HAL data for an EL41x2 terminal.
 *
 * Contains an array of per-channel data structures, one entry per output channel.
 */
typedef struct {
  lcec_el41x2_chan_t chans[LCEC_EL41x2_CHANS]; /**< Per-channel HAL data (2 channels). */
} lcec_el41x2_data_t;
 ...
 * the EtherCAT sync manager and PDO entries for both channels (CoE indices
 * 0x3001 and 0x3002), exports all HAL pins (indexed 0 and 1), and sets
 * default values for scale (1.0), min-dc (-1.0), and max-dc (1.0) on every
 * channel.
 *
...
int lcec_el41x2_init(int comp_id, struct lcec_slave *slave, ec_pdo_entry_reg_t **pdo_entry_regs) {
  ...
  // alloc hal memory
  if ((hal_data = env->hal->malloc(env->hal->ctx, sizeof(lcec_el41x2_data_t))) == NULL) {
...
/**
 * @brief Real-time write callback for the EL41x2.
 *
