/**
 * @file el40x8.c
 * @brief Driver implementation for Beckhoff EL40x8 8-channel analog output terminals.
 *
 * Handles initialization and real-time write processing for the EL4008,
 * EL4018, EL4028, and EL4038 eight-channel analog output terminals.
 * Eight independent channels each scale a HAL float input (via per-channel
...
/**
 * @brief Per-channel HAL data for a single EL40x8 output channel.
 *
...
/**
 * @brief Top-level HAL data for an EL40x8 terminal.
 *
 * Contains an array of per-channel data structures, one entry per output channel.
 */
typedef struct {
  lcec_el40x8_chan_t chans[LCEC_EL40x8_CHANS]; /**< Per-channel HAL data (8 channels). */
} lcec_el40x8_data_t;

static const lcec_pindesc_t slave_pins[] = {
  { GOMC_HAL_FLOAT, GOMC_HAL_IO, offsetof(lcec_el40x8_chan_t, scale), "%s.%s.%s.aout-%d-scale" },
  { GOMC_HAL_FLOAT, GOMC_HAL_IO, offsetof(lcec_el40x8_chan_t, offset), "%s.%s.%s.aout-%d-offset" },
  { GOMC_HAL_FLOAT, GOMC_HAL_IO, offsetof(lcec_el40x8_chan_t, min_dc), "%s.%s.%s.aout-%d-min-dc" },
  { GOMC_HAL_FLOAT, GOMC_HAL_IO, offsetof(lcec_el40x8_chan_t, max_dc), "%s.%s.%s.aout-%d-max-dc" },
  { GOMC_HAL_FLOAT, GOMC_HAL_OUT, offsetof(lcec_el40x8_chan_t, curr_dc), "%s.%s.%s.aout-%d-curr-dc" },
  { GOMC_HAL_BIT, GOMC_HAL_IN, offsetof(lcec_el40x8_chan_t, enable), "%s.%s.%s.aout-%d-enable" },
  { GOMC_HAL_BIT, GOMC_HAL_IN, offsetof(lcec_el40x8_chan_t, absmode), "%s.%s.%s.aout-%d-absmode" },
  { GOMC_HAL_FLOAT, GOMC_HAL_IN, offsetof(lcec_el40x8_chan_t, value), "%s.%s.%s.aout-%d-value" },
  { GOMC_HAL_S32, GOMC_HAL_OUT, offsetof(lcec_el40x8_chan_t, raw_val), "%s.%s.%s.aout-%d-raw" },
  { GOMC_HAL_BIT, GOMC_HAL_OUT, offsetof(lcec_el40x8_chan_t, pos), "%s.%s.%s.aout-%d-pos" },
  { GOMC_HAL_BIT, GOMC_HAL_OUT, offsetof(lcec_el40x8_chan_t, neg), "%s.%s.%s.aout-%d-neg" },
  { GOMC_HAL_TYPE_UNSPECIFIED, GOMC_HAL_DIR_UNSPECIFIED, -1, NULL }
};

static ec_pdo_entry_info_t lcec_el40x8_channel1[] = {
    {0x7000, 1, 16}  // output
};

static ec_pdo_entry_info_t lcec_el40x8_channel2[] = {
    {0x7010, 1, 16}  // output
};

static ec_pdo_entry_info_t lcec_el40x8_channel3[] = {
    {0x7020, 1, 16}  // output
};

static ec_pdo_entry_info_t lcec_el40x8_channel4[] = {
    {0x7030, 1, 16}  // output
};

static ec_pdo_entry_info_t lcec_el40x8_channel5[] = {
    {0x7040, 1, 16}  // output
};

static ec_pdo_entry_info_t lcec_el40x8_channel6[] = {
    {0x7050, 1, 16}  // output
};

static ec_pdo_entry_info_t lcec_el40x8_channel7[] = {
    {0x7060, 1, 16}  // output
};

static ec_pdo_entry_info_t lcec_el40x8_channel8[] = {
    {0x7070, 1, 16}  // output
};

static ec_pdo_info_t lcec_el40x8_pdos_in[] = {
    {0x1600, 1, lcec_el40x8_channel1},
    {0x1601, 1, lcec_el40x8_channel2},
    {0x1602, 1, lcec_el40x8_channel3},
    {0x1603, 1, lcec_el40x8_channel4},
    {0x1604, 1, lcec_el40x8_channel5},
    {0x1605, 1, lcec_el40x8_channel6},
    {0x1606, 1, lcec_el40x8_channel7},
    {0x1607, 1, lcec_el40x8_channel8}
};

static ec_sync_info_t lcec_el40x8_syncs[] = {
    {0, EC_DIR_OUTPUT, 0, NULL},
    {1, EC_DIR_INPUT,  0, NULL},
    {2, EC_DIR_OUTPUT, 8, lcec_el40x8_pdos_in},
    {0xff}
};

void lcec_el40x8_write(struct lcec_slave *slave, long period);

/**
 * @brief Initialize an EL40x8 slave device.
 *
 * Registers the write callback, allocates and zeroes HAL memory, configures
 * the EtherCAT sync manager and PDO entries for all 8 channels (using CoE
 * indices 0x7000–0x7070 in 0x10 steps), exports all HAL pins (indexed 0–7),
 * and sets default values for scale (1.0), min-dc (-1.0), and max-dc (1.0)
 * on every channel.
 *
...
int lcec_el40x8_init(int comp_id, struct lcec_slave *slave, ec_pdo_entry_reg_t **pdo_entry_regs) {
  ...
  lcec_el40x8_data_t *hal_data;
  lcec_el40x8_chan_t *chan;
  int i;
  int err;

  // initialize callbacks
  slave->proc_write = lcec_el40x8_write;

  // alloc hal memory
  if ((hal_data = env->hal->malloc(env->hal->ctx, sizeof(lcec_el40x8_data_t))) == NULL) {
    gomc_log_errorf(env->log, master->instance_name, "hal_malloc() for slave %s.%s failed", master->name, slave->name);
    return -EIO;
  }
  memset(hal_data, 0, sizeof(lcec_el40x8_data_t));
  slave->hal_data = hal_data;

  // initializer sync info
  slave->sync_info = lcec_el40x8_syncs;

  // initialize pins
  for (i=0; i<LCEC_EL40x8_CHANS; i++) {
...
/**
 * @brief Real-time write callback for the EL40x8.
 *
 * Called every servo cycle.  Iterates over all 8 channels and for each one:
...
void lcec_el40x8_write(struct lcec_slave *slave, long period) {
  lcec_master_t *master = slave->master;
  const cmod_env_t *env = master->env;
  lcec_el40x8_data_t *hal_data = (lcec_el40x8_data_t *) slave->hal_data;
