...
//
// Arrays whose sole child is a struct-typed placeholder (promoted during
// parsing) are similarly mapped to the struct type name.
func collectTypeDefs(node *Node, nameSet map[string]int, tm nodeTypeMap, typeDefs *[]genTypeDef, aliases TypeAliasMap) {
	...
	// For an array whose TypeName was promoted from a "struct _ TypeName"
	// element during parsing, use the struct type name directly.
	if node.ArrayStart > 0 && node.TypeName != "" && aliases != nil {
		if alias, ok := aliases[node.TypeName]; ok && alias.StructDef != nil {
			tm[node] = node.TypeName
			return
		}
