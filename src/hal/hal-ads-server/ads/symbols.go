...
type groupAccessor struct {
	...
	overrideTypeName string    // when non-empty, returned by TypeName() instead of typeName
	typeGUID         [16]byte  // returned by TypeGUID()
...
// SetGroupTypeInfo sets the override type name and GUID for a named group symbol.
// This is used when a container node in the config file references a @type alias,
// so that ADSIGRP_SYM_INFOBYNAMEEX (0xF009) responses carry the correct type name
// and data-type GUID for struct/array container symbols.
