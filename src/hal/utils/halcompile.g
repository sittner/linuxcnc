...
""", file=f)
    ...
    for name, fp in functions:
        if name in names:
            Error("Duplicate item name: %s" % name)
        print("static void %s(struct __comp_state *__comp_inst, hal_ctx_t *ctx);" % to_c(name), file=f)
        names[name] = 1
    ...
    for name, fp in functions:
        print("    rtapi_snprintf(buf, sizeof(buf), \"%%s%s\", prefix);"\
            % to_hal("." + name), file=f)
        print("    r = hal_export_funct(buf, (void(*)(void *inst, hal_ctx_t *))%s, inst, %s, 0, comp_id);" % (
...
"""%MAX_USERSPACE_NAMES, file=f)
    ...
    if not options.get("no_convenience_defines"):
        ...
        print("#define FUNCTION(name) static void name(struct __comp_state *__comp_inst, hal_ctx_t *ctx)", file=f)
        ...
        print("#define fperiod (hal_ctx_period(ctx) * 1e-9)", file=f)
        print("#undef period", file=f)
        print("#define period hal_ctx_period(ctx)", file=f)
        for name, type, array, dir, value, personality in pins:
