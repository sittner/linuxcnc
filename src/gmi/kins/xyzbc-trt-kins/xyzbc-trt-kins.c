// xyzbc-trt-kins — 5-axis BC tilting rotary table kinematics (cmod version)
...
static int xyzbc_forward(const double joints[KINS_MAX_JOINTS],
                         kins_pose_t *world)
{
    ...
    double b_r  = joints[JB] * TO_RAD;
    double c_r  = joints[JC] * TO_RAD;

    world->x =   cos(c_r) * cos(b_r) * (joints[JX] - dx - x_rp)
               + sin(c_r)             * (joints[JY]      - y_rp)
               - cos(c_r) * sin(b_r) * (joints[JZ] - dz - z_rp)
               + cos(c_r) * dx + x_rp;

    world->y = - sin(c_r) * cos(b_r) * (joints[JX] - dx - x_rp)
               + cos(c_r)             * (joints[JY]      - y_rp)
               + sin(c_r) * sin(b_r) * (joints[JZ] - dz - z_rp)
               - sin(c_r) * dx + y_rp;

    world->z =   sin(b_r) * (joints[JX] - dx - x_rp)
               + cos(b_r) * (joints[JZ] - dz - z_rp)
               + dz + z_rp;
...
static int xyzbc_inverse(const kins_pose_t *world,
                         double joints[KINS_MAX_JOINTS])
{
    int JA = g_map.principal[3];
    int JU = g_map.principal[6], JV = g_map.principal[7];
    int JW = g_map.principal[8];
    ...
    double dx   = *haldata->x_offset;
    double dz   = *haldata->z_offset + *haldata->tool_offset;
    double b_r  = world->b * TO_RAD;
    double c_r  = world->c * TO_RAD;
    double dpx  = -cos(b_r) * dx - sin(b_r) * dz + dx;
    double dpz  =  sin(b_r) * dx - cos(b_r) * dz + dz;

    kins_pose_t P;
    P.x = + cos(c_r) * cos(b_r) * (world->x - x_rp)
          - sin(c_r) * cos(b_r) * (world->y - y_rp)
          + sin(b_r)             * (world->z - z_rp)
          + dpx + x_rp;

    P.y = + sin(c_r) * (world->x - x_rp)
          + cos(c_r) * (world->y - y_rp)
          + y_rp;

    P.z = - cos(c_r) * sin(b_r) * (world->x - x_rp)
          + sin(c_r) * sin(b_r) * (world->y - y_rp)
          + cos(b_r)             * (world->z - z_rp)
          + dpz + z_rp;
    ...
    uint64_t fflags, uint64_t *iflags, int32_t *out)
{
    ...
    switch (g_sw.current_type) {
        case 0:  *out = xyzbc_forward(joints, world); return 0;
    ...
    uint64_t iflags, uint64_t *fflags, int32_t *out)
{
    ...
    switch (g_sw.current_type) {
        case 0:  *out = xyzbc_inverse(world, joints); return 0;
...
static kins_callbacks_t xyzbc_callbacks = {
...
static cmod_t xyzbc_cmod;

static void xyzbc_destroy(cmod_t *self) {
    (void)self;
        ...
        int argc, const char **argv, cmod_t **out)
{
    ...
    rc = kins_api_register(env->api, "kinematics", &xyzbc_callbacks);
    if (rc != 0) {
        gomc_log_errorf(env->log, name, "kins_api_register failed: %d", rc);
        goto fail;
    }

    xyzbc_cmod.Destroy = xyzbc_destroy;
    *out = &xyzbc_cmod;
    return 0;
