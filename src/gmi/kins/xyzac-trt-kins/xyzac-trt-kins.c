...
static int xyzac_forward(const double joints[KINS_MAX_JOINTS],
                         kins_pose_t *world)
{
...
static int xyzac_inverse(const kins_pose_t *world,
    ...
    uint64_t fflags, uint64_t *iflags, int32_t *out)
{
    ...
    switch (g_sw.current_type) {
        case 0:  *out = xyzac_forward(joints, world); return 0;
    ...
    uint64_t iflags, uint64_t *fflags, int32_t *out)
{
    ...
    switch (g_sw.current_type) {
        case 0:  *out = xyzac_inverse(world, joints); return 0;
        ...
        int argc, const char **argv, cmod_t **out)
{
    ...
    rc = kins_api_register(env->api, "kinematics", &xyzac_callbacks);
    if (rc != 0) {
