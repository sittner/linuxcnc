...
// halparse.INILookup interface required by the halparse parser.
...
// Resolve implements halparse.PathResolver so that Executor can be passed directly
// to halparse.NewMultiFileParser and halparse.NewSingleFileParser.
func (e *Executor) Resolve(path string) (string, error) {
	return e.resolvePath(path)
}

// iniLookup returns a halparse.INILookup adapter, or nil when no INI file is set.
func (e *Executor) iniLookup() halparse.INILookup {
	if e.ini == nil {
		return nil
	}
...
// IniLookup returns a halparse.INILookup adapter for the INI file associated with
// this Executor. Returns nil when no INI file is set.
// This is the public accessor used by callers (e.g. cleanup.go) that need to
// pass an INILookup to halparse.NewSingleFileParser directly.
func (e *Executor) IniLookup() halparse.INILookup {
	return e.iniLookup()
}

// ParseAll reads all [HAL]HALFILE entries from the INI file, parses them,
// and returns the merged ParseResult without executing anything.
//
// The caller controls execution phases via result.Load() and result.Execute().
...
func (e *Executor) ParseAll() (*halparse.ParseResult, error) {
	if e.ini == nil {
		return &halparse.ParseResult{}, nil
	}
	...
	for _, entry := range e.ini.GetSection("HAL") {
		...
		if err != nil {
			return nil, fmt.Errorf("resolving HAL file %q: %w", f, err)
		}
		if strings.HasSuffix(strings.ToLower(resolved), ".tcl") {
			return nil, fmt.Errorf("HAL file %q: TCL HAL files (.tcl) are no longer supported; use .hal files only", resolved)
		}
	...
	mp := halparse.NewMultiFileParser(e.iniLookup(), e)
	result, err := mp.Parse(paths)
	if err != nil {
		return nil, fmt.Errorf("parsing HAL files: %w", err)
	}
	return result, nil
}
...
func (e *Executor) ExecuteHalCommands() error {
	...
	sp := halparse.NewSingleFileParser(e.iniLookup(), e)
