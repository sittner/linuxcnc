...
func (g *generator) hasUserMainloop() bool {
	// Userspace components define user_mainloop() instead of RT FUNCTION()s.
	return g.isUserspace()
}
...
func (g *generator) emitFunctionForwards() {
	if g.isUserspace() {
		// Userspace components define user_mainloop() instead of RT functions.
...
func (g *generator) emitConvenienceDefines() {
	...
	if g.isUserspace() {
		// Userspace: no FUNCTION macro, provide exit helpers and __comp_inst.
		...
		// Exit helpers using the instance's eventfd.
		...
		// FOR_ALL_INSTS — singleton expands to nothing (inst is the global).
		g.printf("#define FOR_ALL_INSTS() /* singleton — __comp_inst is already set */\n\n")
	} else {
		// RT: FUNCTION macro wraps user code into the real function signature.
...
func (g *generator) needsAutoWrap() bool {
	...
	// Userspace components provide user_mainloop(), never auto-wrap.
	if g.isUserspace() {
		return false
	}
...
func (g *generator) emitNew() {
	...
	// Userspace: create eventfd for exit signalling.
	if g.isUserspace() {
	...
	// Export functions (RT only — userspace uses user_mainloop thread).
	if !g.isUserspace() {
		for _, fn := range g.comp.Functions {
