...
// also resolves @enum/@struct aliases via the provided map.
func parseTypeInfoResolved(typeName string, aliases adsconfig.TypeAliasMap) (typeInfo, error) {
	if aliases != nil {
		if alias, ok := aliases[typeName]; ok {
			...
			ti.adsTypeName = typeName
			ti.typeGUID = alias.GUID
			return ti, nil
		}
	}
...
// and GUID on the corresponding groupAccessor in the symbol table.
//
// It must be called after NewBridge has registered all symbols, but before
// the ADS server starts serving requests.
...
func NewBridge(comp *hal.Component, pins []adsconfig.LayoutPin, st *ads.SymbolTable, aliasOpts ...adsconfig.TypeAliasMap) (*Bridge, error) {
	...
	// Compute padded group sizes from layout information.
	...
	// Detect array container groups from bracket notation.
